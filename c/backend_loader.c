...
typedef int            (*fn_available_device_count)(void);  /* optional in older DLLs */
...
static int coli_cuda_load(void){
    ...
    RESOLVE_OPT(available_device_count, fn_available_device_count)
...
int coli_cuda_available_device_count(void){
    if(!g_cuda.available || !g_cuda.available_device_count) return 0;
