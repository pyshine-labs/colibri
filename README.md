...
### Windows, 16 GB VRAM (verified on RTX 4060 Ti): Qwen3.6-35B-A3B

An end-to-end recipe that fits Qwen3.6-35B-A3B (MoE, 256 experts/layer, 3B
active) on a 16 GB card: the CUDA VRAM expert tier holds ~12.7 GB of experts on
the GPU with full RAM residency (`--cap 256`). Verified on Windows 11, driver
560.94:

1. **Model** — the int4-gs64 container, ~20 GB:
   [Kreuzzelg/qwen36-35b-a3b-colibri-i4-gs64](https://huggingface.co/Kreuzzelg/qwen36-35b-a3b-colibri-i4-gs64).
   Verify with `coli doctor --model <dir> --deep` (all 47 files checked).
2. **Build** — MinGW gcc for the engine host, MSVC + nvcc for the GPU DLL:
   ```bash
   make qwen36                                # engine host (qwen36.exe)
   make cuda-dll CUDA_HOME=<CUDA 12.6 root> NVCC=<nvcc>   # coli_cuda.dll
   ```
   Copy `cudart64_12.dll` next to `qwen36.exe`. The CUDA runtime must match the
   driver: a DLL built against CUDA 13.x fails with `cudaError 35
   (InsufficientDriver)` on 560.x drivers — build against CUDA 12.6.
3. **Run** — `run-qwen36.bat` in the repo root (stops any previous instance,
   sets `COLI_CUDA=1`, launches `coli web --cap 256`). Dashboard + OpenAI API
   land on `http://127.0.0.1:8000/` (model id `qwen3.6-colibri`). First start
   takes ~10 min (47 s weight load + expert warmstart filling 15 GB VRAM);
   decode measures ~2.3 tok/s, RSS ~30 GB RAM.

> **Fork notes:** this fork carries two Windows build fixes — the `qwen36`
> Makefile target now compiles the CUDA tier under `CUDA_DLL=1` (previously
> Windows builds silently linked a stub tier), and `backend_loader.c` gained
> the missing `coli_cuda_available_device_count()` forwarder (undefined
> reference with the DLL loader).
